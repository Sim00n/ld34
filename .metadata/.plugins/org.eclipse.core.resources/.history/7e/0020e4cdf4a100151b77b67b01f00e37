...
public class FactorEntity extends Entity {
	...
	public FactorEntity(Texture texture, Texture hoverTexture, float theta, TYPE type, int i, FACTORS factor, boolean active) {
		...
		if(factor == FACTORS.POPULATION_GROWTH) {
			...
			desc = "George Carlin (RIP), one of the best comedians to ever live once said \"the planet’ll shake us off like a bad case of fleas\". We must realize the power of Mother Nature and protect ourselve from its deadly forces.";
			stats = "\n\nInfrastructure: +2.4\nTechnology: +1.2\nInternet Access: +1.4";
		} else {
			icon = new Texture(Gdx.files.internal("textures/icons/people.png"));
			title = "";
